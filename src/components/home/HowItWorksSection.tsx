    ...
    <section className="relative py-24 overflow-hidden" style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)" }}>
      ...
      <div className="relative max-w-5xl mx-auto px-6 z-10">
        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={vp}
          className="text-center text-emerald-400 text-sm font-semibold tracking-widest uppercase mb-4">How It Works</motion.p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
          ...
          <div className="hidden lg:block absolute top-12 left-[15%] right-[15%] h-px bg-gradient-to-r from-emerald-500/30 via-emerald-400/50 to-emerald-500/30 z-0" />
          {steps.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={vp} transition={{ delay: i * 0.1 }}
              className="text-center relative z-10">
              <div className="w-24 h-24 rounded-2xl border border-white/10 bg-white/[0.07] backdrop-blur flex items-center justify-center mx-auto mb-6">
                <s.icon size={32} className="text-emerald-400" />
              </div>
              <span className="text-emerald-400 text-xs font-bold tracking-widest uppercase" style={{ fontFamily: "var(--font-tech)" }}>Step {s.step}</span>
              <h3 className="font-semibold text-white text-[15px] mt-2 mb-2">{s.title}</h3>
              <p className="text-sm text-slate-400 leading-relaxed">{s.desc}</p>
            </motion.div>
          ))}
        </div>
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={vp} className="mt-12 text-center">
          <a href="https://app.lcadesk.com/auth/signup?role=filer" className="inline-flex items-center justify-center gap-2 rounded-xl bg-white text-slate-900 px-8 py-3 text-sm font-semibold hover:bg-emerald-50 hover:shadow-lg transition-all">
            Start Free — No Card Required
          </a>
          <p className="text-xs text-slate-500 mt-3">30-day free trial. No card required. Cancel anytime.</p>
        </motion.div>
      </div>
    </section>
  );
