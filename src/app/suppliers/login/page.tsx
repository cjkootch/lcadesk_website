  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "https://app.lcadesk.com"}/api/public/suppliers/login`,
        {
          method: "POST",
      ...
      await fetch("/api/auth/set-cookie", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token: data.token, userType: "supplier" }),
      });
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <div className="text-center mb-8">
            ...
            <h1 className="font-display text-3xl text-text-primary mb-3">
              Supplier <span className="gradient-text-static">Log In</span>
            </h1>
            <p className="text-text-secondary text-sm">
              Access your supplier profile and procurement alerts.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="bg-card rounded-2xl border border-border p-8 shadow-sm space-y-5">
            ...
            <p className="text-xs text-text-muted text-center pt-2">
              ...
              <Link href="/suppliers/register" className="text-accent hover:underline font-medium">Register here</Link>
            </p>
          </form>
        </motion.div>
      </div>
